...
func TestDiscovery(t *testing.T) {
	...
	t.Run("DefaultOptionErrorFails", func(t *testing.T) {
		...
		d := inproc.Discoverer{Strategy: failDefaultOptions{}}
	...
	t.Run("ValidationErrorFails", func(t *testing.T) {
		...
		d := inproc.Discoverer{Strategy: failValidaton{}}
		peers, err := d.FindPeers(context.Background(), "")
		require.EqualError(t, err, "test")
		require.Nil(t, peers)
	})
...
func TestStrategy(t *testing.T) {
	...
	t.Run("SelectRandom", func(t *testing.T) {
		...
		t.Run("Reproducible", func(t *testing.T) {
			...
			as0, err := runStrategy(env, &inproc.SelectRandom{
				Src: rand.NewSource(42),
			})
			require.NoError(t, err)

			as1, err := runStrategy(env, &inproc.SelectRandom{
				Src: rand.NewSource(42),
			})
			require.NoError(t, err)
	...
	t.Run("SelectRing", func(t *testing.T) {
		...
		t.Run("PeerNotInEnvironmentFails", func(t *testing.T) {
			...
			var s inproc.SelectRing
			as, err := runStrategy(env, s, inproc.WithPeerID(randID()))
			require.Error(t, err)
			require.Nil(t, as)
		})
			...
			for i, a := range peers {
				g.Go(func(i int, a multiaddr.Multiaddr) func() error {
					return func() (err error) {
						...
						if as, err = runStrategy(env, s, inproc.WithPeerID(info.ID)); err == nil {
							neighbors[i] = as[0]
						}
...
func runStrategy(env inproc.PeerListProvider, s inproc.DiscoveryStrategy, opt ...discovery.Option) (inproc.AddrSlice, error) {
	opts := newOption()
...
type failValidaton struct{ inproc.SelectAll }

func (failValidaton) Validate(*discovery.Options) error {
	return errors.New("test")
}

type failDefaultOptions struct{ inproc.SelectAll }
